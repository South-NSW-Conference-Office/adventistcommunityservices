...
/**
 * Format address from locations array
 */
function formatAddress(locations?: ServiceLocation[]): string {
  if (!locations || locations.length === 0) return 'Address not available';
...
/**
 * Format operating hours from scheduling
 */
function formatHours(scheduling?: ServiceScheduling): string {
  if (!scheduling?.weeklySchedule?.schedule) return 'Contact for hours';

  const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

  const enabledDays = scheduling.weeklySchedule.schedule
    .filter((day) => day.isEnabled && day.timeSlots.length > 0)
    .map((day) => {
      const slot = day.timeSlots[0];
      return `${dayNames[day.dayOfWeek]}: ${slot.startTime} - ${slot.endTime}`;
    });
...
export function ServiceDetails() {
  ...
  const phone = service.contactInfo?.phone || 'Contact for details';
              ...
              {showBookingForm && (
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6">
                  ...
                  <form className="space-y-4">
                    ...
                    <div>
                      <label className="block text-white/90 text-sm mb-2">Preferred Date</label>
                      <input
                        type="date"
                        className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white focus:outline-none focus:border-white/40"
                      />
                    </div>
                    <div>
                      <label className="block text-white/90 text-sm mb-2">Message</label>
                      <textarea
                        rows={4}
                        className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/50 focus:outline-none focus:border-white/40"
